...
Só **4 services de dados** (`AuthService`, `ChildrenService`, `VaccinationService`, `CampaignService`) importam `firebase/*`. Páginas e componentes consomem observables/signals tipados, sem saber a origem do dado. A lógica de domínio — `ScheduleService` e `computeStatus` — é **pura** e testável sem Firebase (mock dos services). Assim a regra de status (joia da coroa) tem cobertura de testes independente da infraestrutura. Ver [ADR-004](adr-004-persistencia-firestore.md) e [ADR-005](adr-005-modelagem-status.md).
